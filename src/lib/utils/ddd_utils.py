...
def draw_box_3d(image, corners, c=(0, 0, 255)):
  ...
  for ind_f in range(3, -1, -1):
    f = face_idx[ind_f]
    for j in range(4):
      #print((corners[f[j], 0], corners[f[j], 1]), (corners[f[(j+1)%4], 0], corners[f[(j+1)%4], 1]), c, 2)
      cv2.line(image, (int(corners[f[j], 0]), int(corners[f[j], 1])), (int(corners[f[(j+1)%4], 0]), int(corners[f[(j+1)%4], 1])), c, 2, lineType=cv2.LINE_AA)
    if ind_f == 0:
